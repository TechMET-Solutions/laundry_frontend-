...
const AddServicesModel = ({ mode, onClose, serviceData, setRefresh }) => {
  ...
  const addService = () => {
    setServices((prev) => [...prev, { type: "", price: "", image: "" }]);
  };
  ...
  const handleServiceChange = (index, field, value) => {
    ...
    if (field === "type") {
      // Find the original object to get the image
      const selectedTypeObj = serviceTypes.find(type => type.name === value);

      updated[index] = {
        ...updated[index],
        type: value,
        // Store the image path from the API response
        image: selectedTypeObj ? selectedTypeObj.image : ""
      };
    } else {
      updated[index][field] = value;
    }
...
export default AddServicesModel;